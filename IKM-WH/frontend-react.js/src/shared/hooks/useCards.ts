...
export const useCards = () => {
  ...
  const fetchCardSets = async () => {
    try {
      ...
      if (response.success) {
        // Фильтруем только карточки с доступными наборами для клиентской части
        setCardSets(response.data.filter(cardSet => cardSet.setsAvailable > 0));
      } else {
  ...
  const createCardSet = async (data: CreateCardSetData, imageFile?: File) => {
    try {
      ...
      if (response.success) {
        setCardSets(prev => [...prev, response.data]);
